...
public class LoggingTest {
    ...
    public void confirmLoggerClosed() throws Exception {
        ...
        ch.qos.logback.classic.Logger sqliteInfo = (ch.qos.logback.classic.Logger) LoggerFactory
                .getLogger("io.supertokens.storage.sqlite.Info." + process.getProcess().getProcessId());
        ch.qos.logback.classic.Logger sqliteError = (ch.qos.logback.classic.Logger) LoggerFactory
                .getLogger("io.supertokens.storage.sqlite.Error." + process.getProcess().getProcessId());

        ch.qos.logback.classic.Logger hikariLogger = (Logger) LoggerFactory.getLogger("com.zaxxer.hikari");

        assertTrue(sqliteInfo.iteratorForAppenders().hasNext());
        assertTrue(sqliteError.iteratorForAppenders().hasNext());
        assertTrue(hikariLogger.iteratorForAppenders().hasNext());

        process.kill();
        assertNotNull(process.checkOrWaitForEvent(ProcessState.PROCESS_STATE.STOPPED));

        assertTrue(!sqliteInfo.iteratorForAppenders().hasNext() && !sqliteError.iteratorForAppenders()
                .hasNext());
